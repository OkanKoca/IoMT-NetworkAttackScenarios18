...
farklı hiçbir etki üretmediğinin ikinci bir kanıtıdır — ve bu kanıt, senaryo kodunu hiç
...
çalışmanın kendi verisindeki gözlem de bunu desteklemektedir — orada `mqtt` koşularının
öznitelik vektörleri `normal` koşularınkiyle 10 tohumun 10'unda da özdeştir (§2.4).
...
> §6'daki rakamlar doğrudur, ama sordukları soru sanıldığı soru değildir. Bu bölüm üç
> kontrol deneyi anlatmaktadır; üçü de aynı biçimde kurulmuştur — **ölçülen etkiyi, onunla
> birlikte değişen ama fark edilmeyen bir etkenden ayırmak.** Üçü de aynı sonuca varmaktadır
> ve o sonuç §8.4'te toplanmıştır. Bu bölüm raporun asıl katkısıdır.

### 8.1 Detektör saldırıyı mı, aracının varlığını mı görüyor?

**Sorunun kaynağı.** §6.3'te grey-hole kolunun tespit eğrisi düzdür: `p = 0.02`'de bile tespit
1.00'dır, yani paketlerin yalnızca %2'si düşürüldüğünde bile saldırı kusursuz yakalanmaktadır.
Bu, gerçek olamayacak kadar iyidir. §7.3'teki doğrulama tablosu nedenini sezdirmektedir:
`p = 0.02`'de teslim oranı 0.895'tir, oysa saldırının kendisi yalnızca %2 kayıp eklemektedir.
Geri kalan kaybı ne üretmektedir?
...
üretmektedir. Dahası, model bunları rastgele bir sınıfa atamamaktadır: 36'sına **`greyhole`**
demektedir (2 `dos`, 1 `normal`, 1 `blackhole`).

**Bu, grey-hole eğrisinin neden düz olduğunu tamamen açıklamaktadır.** Eğri 1.00'da
başlamaktadır çünkü `p = 0`'da zaten 0.975'tedir. Saldırının şiddetini artırmanın
oynatabileceği pay yalnızca **0.025**'tir.

**Aynı sonuç öznitelik uzayında da doğrulanmaktadır.** Kurban yolunun teslim oranı üç aşamada
izlendiğinde (üç kolun da paylaştığı 10 tohum üzerinden):
...
*(Bu üç ortalama aynı tohum kümesi üzerinden alınmıştır ve bu bir ayrıntı değildir. Kollar
farklı tohum kümelerinde ölçülüp çıkarıldığında saldırının katkısı **pozitif** çıkmaktadır —
yani "%2 düşüren aracı, hiç düşürmeyenden daha çok teslim ediyor" gibi fiziksel olarak imkânsız
bir sonuç. Fark gerçek değil, karşılaştırma hatasıdır.)*

**Sonuç.** "Saldırı tespit edildi" çıktısının büyük kısmı, saldırının kendisinden değil ağ
yolunda fazladan bir düğüm bulunmasından gelmektedir. Detektörün fiilen yanıtladığı soru
**"bu ağda beklenmedik bir aracı var mı?"**dır — "bu aracı kötü niyetli mi?" değil.

#### Doğru soru sorulduğunda ortaya çıkan gerçek çöküş eğrisi
...
**Konum eşleştirmesi bu karşılaştırmada pazarlık dışıdır ve bunun sebebi ölçülmüştür.** İlk
...
Bu bir tespit başarısızlığı gibi görünmektedir, fakat değildir — ve farkı ayırt etmek §8'in
yöntemidir. §2.4'te ölçülen üç olgu bu sonucu tamamen açıklamaktadır: o veride altı etiket
...
> Bu bölüm, yukarıdaki sonuçların geçerli olmadığı yerleri sıralar. Bir kısmı §8'de
> ölçülmüştür ve burada toplanmaktadır; bir kısmı ölçülmemiş ama bilinen sınırlardır.
> Sıralama önem derecesine göredir.
...
> Bu bölüm, yukarıdaki hiçbir rakamın elle kopyalanmadığını ve yayımlanan sürümün kendi
> iddialarını makinece doğruladığını gösterir. Bu bir biçimsellik değildir: çalışmanın
> gerçekten düştüğü hataların çoğu ölçüm hatası değil **kayıt hatası** olmuştur, ve
