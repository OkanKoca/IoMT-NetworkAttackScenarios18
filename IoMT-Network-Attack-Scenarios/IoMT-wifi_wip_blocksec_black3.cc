...
bool MitmInterceptAndDrop(Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocol, const Address &from) {
    if (protocol == 17) { // 17 = UDP
        ...
        pktCopy->RemoveHeader(ipv4Header);  // You need the IP header to get the destination address

        Ipv4Address dstAddr = ipv4Header.GetDestination();
        uint16_t dstPort = udpHeader.GetDestinationPort();

        if (dstAddr == Ipv4Address("192.168.1.1") && dstPort == 8080) {
            NS_LOG_UNCOND("MITM Attack: Dropping packet to Baxter (192.168.1.1:8080)");
...
void SetupMitmAttack(NodeContainer &attackerNode, NodeContainer &wifiNodes, Ipv4InterfaceContainer &staInterfaces) {
    ...
    if (attackerRouting) {
        Ipv4Address targetAddress = staInterfaces.GetAddress(0);
...
int main(int argc, char *argv[])
{
    ...
    std::ofstream testFile("simulation_test_output_ddos.txt");
    ...
    std::string flowOutputPath = "flowmonitor-stats_wip_blocksec_black3.xml";

    CommandLine cmd;
    cmd.AddValue("attackType", "Type of attack: none / blackhole", attackType);
    ...
    SetupMitmAttack(attackerNode, wifiNodes, staInterfaces);
   ...
   phy.EnablePcap("baxter-infusion-pump_blocksec_black", staDevices.Get(0));
   ...
   phy.EnablePcap("hexoskin-smartphone_blocksec_black", staDevices.Get(1));
