...
// --- MQTT FLOOD TAG ---
class MqttFloodTag : public Tag
{
public:
    MqttFloodTag() {}
    static TypeId GetTypeId(void) {
        static TypeId tid = TypeId("MqttFloodTag")
            .SetParent<Tag>()
            .AddConstructor<MqttFloodTag>();
        return tid;
    }
    virtual TypeId GetInstanceTypeId(void) const override { return GetTypeId(); }
    virtual void Serialize(TagBuffer i) const override { i.WriteU8(1); }
    virtual void Deserialize(TagBuffer i) override { i.ReadU8(); }
    virtual uint32_t GetSerializedSize(void) const override { return 1; }
    virtual void Print(std::ostream &os) const override { os << "MQTT Flood"; }
};

void SendMqttFloodPacket(Ptr<Socket> socket)
{
...
void RxCallback(Ptr<const Packet> packet, const Address &from)
{
    ...
    if (packet->PeekPacketTag(tag)) {
        std::cout << "[!] Received MQTT Flood packet!" << std::endl;
    }
}
...
double wipCpuUsage = 0.0;       // Simulated CPU usage (e.g., in %)
double wipMemoryUsage = 0.0;    // Simulated memory usage (e.g., in MB)

void WipUsageCallback(Ptr<Socket> socket)
{
    Ptr<Packet> packet;
    Address from;
    
    static double totalDataProcessed = 0;
    totalDataProcessed += packet->GetSize();

    double cpuLoadPercent = std::min(100.0, (totalDataProcessed / 1000000.0) * 100); // Simulate CPU usage
    std::cout << "Simulated CPU Load: " << cpuLoadPercent << "%" << std::endl;
    
    static double memoryUsedKb = 0;
    memoryUsedKb += packet->GetSize() / 1024.0;
    std::cout << "Simulated Memory Usage: " << memoryUsedKb << " KB" << std::endl;

    while ((packet = socket->RecvFrom(from)))
    {
        // Optionally print received size
        NS_LOG_INFO("WIP received " << packet->GetSize() << " bytes from " << InetSocketAddress::ConvertFrom(from).GetIpv4());

        // Simulate CPU or memory usage tracking here
        // Example: log timestamp and size
        double now = Simulator::Now().GetSeconds();
        std::cout << "At " << now << "s: WIP received " << packet->GetSize() << " bytes." << std::endl;

        // You can simulate memory load tracking by adding to a global counter, etc.
    }
}

void LogWipUsage()
{
    NS_LOG_UNCOND("Periodic WIP Resource Log -> CPU: " << wipCpuUsage 
                    << " ms | Memory: " << wipMemoryUsage << " MB");
    Simulator::Schedule(Seconds(5.0), &LogWipUsage); // Repeat every 5s
}
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "START OF NS-3 SIMULATION" << std::endl;

    std::ofstream testFile("simulation_test_output_mqtt.txt");
    testFile << "NS-3 simulation output guaranteed!" << std::endl;
    testFile.close();

    //Simulator::Schedule(Seconds(60.0), []() {
      //  std::cout << "NS-3 Simulation Event Triggered!" << std::endl;
    //});
    ...
    std::string attackType    = "mqtt-attack";
    uint32_t    seedValue     = 1;
    std::string flowOutputPath = "flowmonitor-stats_wip_blocksec_mqtt.xml";

    CommandLine cmd;
    cmd.AddValue("attackType", "Type of attack: none / mqtt-attack", attackType);
    ...
    NodeContainer wifiApNode, wifiNodes, hexoskinNodes, mitmNode;
    ...
    Ptr<Socket> attackerSocket = Socket::CreateSocket(mitmNode.Get(0), UdpSocketFactory::GetTypeId());
    // Schedule attacker event at the very last moment
    Simulator::Schedule(Seconds(simulationTime - 0.01), &SendMqttFloodPacket, attackerSocket);

    // Connect Rx callback to PacketSink
    ...
    phy.EnablePcap("wifi-ap_blocksec_mqtt", apDevice);
    phy.EnablePcap("baxter_blocksec_mqtt", staDevices.Get(0));
    p2p.EnablePcap("hexoskin_blocksec_mqtt", p2pDevices);
    phy.EnablePcap("hexoskin_phone_blocksec_mqtt", staDevices.Get(1));
    phy.EnablePcap("mitm_mqtt_node_blocksec_mqtt", mitmNode);

    AnimationInterface anim("network-anim_wip_blocksec_mqtt.xml");
