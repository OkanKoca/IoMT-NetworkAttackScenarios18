...
// Function to simulate a MITM blackhole attack (drops Hexoskin Shirt traffic)
bool MitmInterceptAndDrop(Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocol, const Address &from) {
    // Check if packet belongs to the Hexoskin Shirt (e.g., by port number)
    if (protocol == 17 && from == InetSocketAddress("10.1.1.2", 8070)) { // UDP packet to Hexoskin Shirt
        NS_LOG_UNCOND("MITM Attack: Intercepted Hexoskin packet!");
        return false; // Drop the packet
    }
    return true; // Allow other packets
}

// Set up the MITM-based blackhole attack function

void SetupMitmAttack(NodeContainer &attackerNode, NodeContainer &hexoskinNodes, Ipv4InterfaceContainer &p2pInterfaces) {
    Ptr<Node> attacker = attackerNode.Get(0);

    Ipv4Address targetAddress = p2pInterfaces.GetAddress(1); // Hexoskin node IP
    ...
    if (attackerRouting) {
        attackerRouting->SetDefaultRoute(targetAddress, 1);
    }

    // Set receive callback on Hexoskin's NetDevice
    Ptr<NetDevice> hexoskinDevice = hexoskinNodes.Get(0)->GetDevice(0);
    hexoskinDevice->SetReceiveCallback(MakeCallback(&MitmInterceptAndDrop));
}
...
int main(int argc, char *argv[])
{
    ...
    std::ofstream testFile("simulation_test_output_mitm.txt");
    ...
    std::string attackType    = "blackhole";               // “none” or “blackhole”
    uint32_t    seedValue     = 1;                    // RNG seed
    std::string flowOutputPath = "flowmonitor-stats_hex_blocksec_black3.xml";
    ...
    // Create Attacker Node for MITM Blackhole attack
    NodeContainer attackerNode;
    attackerNode.Create(1); // One attacker node
    ...
    stack.Install(attackerNode); // Install stack on attacker node
    ...
    // Set up mobility for the Attacker Node (position between Hexoskin and target)
    Ptr<ListPositionAllocator> attackerPosition = CreateObject<ListPositionAllocator>();
    attackerPosition->Add(Vector(5.0, 5.0, 0.0)); // Attacker placed between nodes
    ...
    // Set up the MITM-based blackhole attack targeting Hexoskin SHS node
    SetupMitmAttack(attackerNode, hexoskinNodes, p2pInterfaces);
    ...
    phy.EnablePcap("wifi_ap_black", wifiApNode);  // Capture Wi-Fi traffic
    phy.EnablePcap("baxter_pump_black", staDevices.Get(0));  // Capture Baxter traffic (Node 0)
    phy.EnablePcap("hexoskin_phone_black", staDevices.Get(1));  // Capture Baxter traffic (Node 0)
    p2p.EnablePcap("hexoskin_black", p2pDevices);  // Capture Hexoskin traffic (P2P link)

    // Enable NetAnim trace
    AnimationInterface anim("network-anim_black.xml");
