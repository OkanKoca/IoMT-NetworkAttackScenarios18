...
int main(int argc, char *argv[]) {
    ...
    std::string attackType    = "dos";               // “none” or “dos”
    uint32_t    seedValue     = 1;                    // RNG seed
    std::string flowOutputPath = "flowmonitor-stats_hex_blocksec_dos3.xml";

    CommandLine cmd;
    cmd.AddValue("attackType", "Type of attack: none / dos", attackType);
    ...
    // Number of nodes
    uint32_t numWifiNodes = 9; // 9 Wi-Fi devices
    ...
    //Create DoS Attacker Nodes
    NodeContainer attackerNodes;  // Create a container for attacker nodes
    uint32_t numAttackerNodes = 1;  // Number of attacker nodes in the DoS attack
    attackerNodes.Create(numAttackerNodes);  // Create attacker nodes
    
    //NAMING NODES
    Names::Add("Node_AccessPoint", wifiApNode.Get(0));
    Names::Add("Node_Baxter", wifiNodes.Get(0));
    Names::Add("Node_Smartphone", wifiNodes.Get(1));
    Names::Add("Node_Control", wifiNodes.Get(2));
    Names::Add("Node_Attacker", wifiNodes.Get(8));
    Names::Add("Node_Hexoskin", hexoskinNodes.Get(0));
    ...
    Ptr<Ipv4> ipv4 = hexoskinNodes.Get(0)->GetObject<Ipv4>();
    if (ipv4->GetNInterfaces() > 1) {
    ipv4->SetForwarding(1, true);
    }
    
    // --- Begin Refined DoS Attack Simulation with Traffic Realism ---

    Ptr<Node> targetNode = hexoskinNodes.Get(0);   // Target node (Baxter Pump)
    Ipv4Address victimAddress = p2pInterfaces.GetAddress(1);  // Victim IP
    InetSocketAddress remoteSocketAddr(victimAddress, bluetoothPort);

    double attackStart = 10.0;
    double simStopTime = 60.0;

    for (uint32_t i = 0; i < attackerNodes.GetN(); ++i)
    {
        // Randomize packet size between 512 and 1500 bytes
        uint32_t pktSize = 512 + (rand() % (1500 - 512 + 1));

       // Randomize data rate between 5Mbps and 20Mbps
       std::ostringstream rateStream;
       rateStream << (5 + (rand() % 16)) << "Mbps";

       // Create a new OnOffHelper for each attacker with burst variability
       OnOffHelper attackHelper("ns3::UdpSocketFactory", remoteSocketAddr);
       attackHelper.SetAttribute("PacketSize", UintegerValue(pktSize));
       attackHelper.SetAttribute("DataRate", DataRateValue(DataRate(rateStream.str())));
       attackHelper.SetAttribute("OnTime", StringValue("ns3::UniformRandomVariable[Min=0.05|Max=0.2]"));
       attackHelper.SetAttribute("OffTime", StringValue("ns3::UniformRandomVariable[Min=0.01|Max=0.1]"));

       // Install and start application with jittered start time
       ApplicationContainer app = attackHelper.Install(attackerNodes.Get(i));
       double jitter = ((rand() % 10) / 100.0);  // 0 to 0.09s
       app.Start(Seconds(attackStart + i * 0.1 + jitter));
       app.Stop(Seconds(simStopTime));
   }
   
   // --- PATCHED BLOCKCHAIN TAMPERING IN DoS SECTION ---
    // Simulate tampering on attacker node (e.g., node 2)
    uint32_t attackerNodeId = 2;

    if (attackerNodeId < nodeBlockchain.size()) {
        Blockchain& attackerBc = nodeBlockchain[attackerNodeId];
        if (attackerBc.getChain().size() > 1) {
            // Tamper with data of block 1 (after genesis)
            Block tamperedBlock = attackerBc.getChain()[1];
            tamperedBlock.data = "Tampered Data - MALICIOUS!";
            tamperedBlock.hash = tamperedBlock.CalculateHash();
            tamperedBlock.digitalSignature = ""; // Invalidate signature

            // Replace block 1 in chain (simple version)
            std::vector<Block> tamperedChain = attackerBc.getChain();
            tamperedChain[1] = tamperedBlock;
            attackerBc.replaceChain(tamperedChain);

            NS_LOG_WARN("Blockchain tampering simulated on node " << attackerNodeId);
        }
    }
  ...
  // --- END PATCH ---
    ...
    phy.EnablePcap("baxter_pump_dos", staDevices.Get(0));  // Capture Baxter traffic (Node 0)
    phy.EnablePcap("hexoskin_phone_dos", staDevices.Get(1));  // Capture Hexoskin Smartphone (Node 0)
...
for (uint32_t i = 0; i < attackerNodes.GetN(); ++i) {
    Ptr<Node> node = attackerNodes.Get(i);
    for (uint32_t j = 0; j < node->GetNApplications(); ++j) {
