...
int main(int argc, char *argv[]) {
    ...
    std::string attackType    = "ddos";               // “none” or “ddos”
    uint32_t    seedValue     = 1;                    // RNG seed
    std::string flowOutputPath = "flowmonitor-stats_wip_blocksec_ddos3.xml";

    CommandLine cmd;
    cmd.AddValue("attackType", "Type of attack: none / ddos", attackType);
    ...
    uint32_t numAttackerNodes = 7;  // Number of attacker nodes in the DDoS attack
    ...
    // --- Begin Refined DDoS Attack Simulation with Traffic Realism ---

    Ptr<Node> targetNode = wifiNodes.Get(0);   // Target node (Baxter Pump)
    Ipv4Address victimAddress = staInterfaces.GetAddress(0);  // Victim IP
    InetSocketAddress remoteSocketAddr(victimAddress, baxterPort);
   ...
   // --- PATCHED BLOCKCHAIN TAMPERING IN DDoS SECTION ---
  ...
  // --- END PATCH ---
    ...
    phy.EnablePcap("baxter_pump_ddos", staDevices.Get(0));  // Capture Baxter traffic (Node 0)
    phy.EnablePcap("hexoskin_phone_ddos", staDevices.Get(1));  // Capture Hexoskin Smartphone (Node 0)
    p2p.EnablePcap("hexoskin_ddos", p2pDevices);  // Capture Hexoskin traffic (P2P link)
 
    // Enable NetAnim trace
    AnimationInterface anim ("network-anim_blocksec_ddos.xml");
